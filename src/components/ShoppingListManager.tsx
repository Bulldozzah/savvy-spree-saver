...
export const ShoppingListManager = ({
  ...
  const getStoreLocationText = () => {
    if (!storeContactInfo) return "";
    const assignedStore = stores.find(s => s.id === assignedStoreId);
    const storeName = assignedStore ? `${assignedStore.store_hq?.name || "Store"} - ${assignedStore.location}` : "Store";
    let text = `📍 ${storeName}\n`;
    if (storeContactInfo.address) text += `Address: ${storeContactInfo.address}\n`;
    const mapUrl = getStoreMapUrl();
    if (mapUrl) text += `\nOpen in Maps: ${mapUrl}`;
    return text;
  };
  ...
  const checkStoreLocation = () => {
    if (!storeContactInfo) {
      toast({ title: "Store Not Selected", description: "Please assign a store first", variant: "destructive" });
      return false;
    }
    if (!storeContactInfo.latitude && !storeContactInfo.longitude && !storeContactInfo.address && !storeContactInfo.location) {
      toast({ title: "Location Not Available", description: "This store doesn't have address or coordinates on file", variant: "destructive" });
      return false;
    }
    return true;
  };
  ...
  const handleOpenInMaps = () => {
    if (!checkStoreLocation()) return;
    const url = getStoreMapUrl();
    if (url) window.open(url, '_blank');
  };

  const handleShareMapWhatsApp = () => {
    if (!checkStoreLocation()) return;
    const text = encodeURIComponent(getStoreLocationText());
    window.open(`https://wa.me/?text=${text}`, '_blank');
  };

  const handleShareMapNative = async () => {
    if (!checkStoreLocation()) return;
    const locationText = getStoreLocationText();
    const mapUrl = getStoreMapUrl();
    if (navigator.share) {
      try {
        await navigator.share({
      ...
      toast({ title: "Copied", description: "Store location copied to clipboard" });
    }
  };
              ...
              <div className="space-y-3 pt-4 border-t">
                ...
                <div className="pt-3 border-t">
                  ...
                  <div className="space-y-2">
                    ...
                    <Button
                      onClick={handleShareMapWhatsApp}
                      className="w-full"
                      variant="outline"
                      disabled={!assignedStoreId}
                    >
                      <MessageCircle className="mr-2 h-4 w-4" />
                      Share Map via WhatsApp
                    </Button>
                    <Button
                      onClick={handleShareMapNative}
                      className="w-full"
                      variant="outline"
                      disabled={!assignedStoreId}
                    >
            ...
            <div className="space-y-3 pt-4 border-t">
              ...
              {assignedStoreId && (
                <div className="pt-3 border-t">
                  ...
                  <div className="space-y-2">
                    <Button onClick={handleOpenInMaps} className="w-full" variant="outline">
                      <MapPin className="mr-2 h-4 w-4" />
                      Open in Google Maps
                    </Button>
                    <Button onClick={handleShareMapNative} className="w-full" variant="outline">
                      <Share2 className="mr-2 h-4 w-4" />
                      Share Map via App...
                    </Button>
                    <Button onClick={handleCopyMapLink} className="w-full" variant="outline">
                      <Mail className="mr-2 h-4 w-4" />
                      Copy Map Link
                    </Button>
                  </div>
