...
<<<<<<< HEAD
  const getStoreMapUrl = () => {
    ...
    if (selectedStore.latitude && selectedStore.longitude) {
      return `https://www.google.com/maps?q=${selectedStore.latitude},${selectedStore.longitude}`;
    }
    if (selectedStore.location) {
      return `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(selectedStore.location)}`;
    }
    return null;
  };
  ...
  const getStoreLocationText = () => {
    ...
    let text = `\ud83d\udccd ${selectedStore.store_hq?.name || "Store"} - ${selectedStore.location}\n`;
    if (selectedStore.address) text += `Address: ${selectedStore.address}\n`;
  ...
  const handleOpenInMaps = () => {
    ...
    else toast({ title: "No Location", description: "This store has no map data available", variant: "destructive" });
  };
  ...
  const handleShareMapNative = async () => {
    ...
    if (navigator.share) {
      try {
        await navigator.share({ title: "Store Location", text: locationText, url: mapUrl || undefined });
      } catch { /* cancelled */ }
    } else {
...
<<<<<<< HEAD
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-52">
              ...
              <DropdownMenuItem onClick={handleShareMapWhatsApp}>
                <MessageCircle className="mr-2 h-4 w-4" />
                Share via WhatsApp
              </DropdownMenuItem>
              <DropdownMenuItem onClick={handleShareMapNative}>
